    ...
    public static class Program
    {
        ...
        private static void CleanTempFiles()
        {
            try
            {
                ...
                {
                    foreach (var f in Directory.GetFiles(TargetDir, "*.tmp", SearchOption.AllDirectories))
                    {
        ...
        private static void ExtractWithRetry(ZipArchiveEntry entry, string dest, int maxRetries = 5)
        {
        ...
        private static void StopService()
        {
            try
            {
                using var p = Process.Start(new ProcessStartInfo("net", "stop VirtualPrinterService")
                {
                    CreateNoWindow = true,
