...
bool PortContext::EnsureTempDir()
{
    ...
    if (FAILED(SHGetFolderPathW(0, CSIDL_COMMON_APPDATA, 0, 0, progData)))
        return false;

    std::wstring dir = std::wstring(progData) + L"\\VirtualPrinter\\";
...
BOOL PortContext::StartDoc(LPWSTR pszPrinterName, DWORD JobId, PDOC_INFO_1W pDocInfo)
{
    ...
    if (FAILED(SHGetFolderPathW(0, CSIDL_COMMON_APPDATA, 0, 0, progData)))
    {
        DbgTrace(L"StartDoc: SHGetFolderPathW FAILED err=%lu elapsed=%lldus", GetLastError(), GetElapsedUs(t0));
        return FALSE;
    }
    DbgTrace(L"StartDoc: progData=%s elapsed=%lldus", progData, GetElapsedUs(t0));

    wchar_t buf[64];
    swprintf_s(buf, L"%d.tmp", JobId);
    m_tempFile = std::wstring(progData) + L"\\VirtualPrinter\\" + buf;
