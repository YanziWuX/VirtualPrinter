...
namespace VirtualPrinter.SaveDialog
{
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            // Copy captured PS file to jobs dir as .pending file
        ...
        private static void CleanupTempFiles()
        {
            try
            {
                ...
                {
                    foreach (var f in Directory.GetFiles(dir, "*.tmp", SearchOption.AllDirectories))
                    {
                        try { File.Delete(f); } catch { }
                    }
                    foreach (var tmpDir in Directory.GetDirectories(dir, "tmp", SearchOption.TopDirectoryOnly))
                    {
        ...
        private void UpdateMultiPageForFormat()
        {
            ...
            if (fmt == "pdf")
            {
                ...
                RasterizePdfCheck.Visibility = Visibility.Visible;
            ...
            else
            {
                ...
                RasterizePdfCheck.Visibility = Visibility.Collapsed;
        ...
        private async void OnPrint(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var opts = new GSConvertOptions
                {
                    ...
                    RasterizeToPdf = RasterizePdfCheck.IsChecked == true,
                ...
                if (gsResult.Success)
                {
                    ...
                    await ServiceClient.SendResultAsync(_jobId, outputPath, true, null);
                    _resultSent = true;
                ...
                else
                {
                    ...
                    await ServiceClient.SendResultAsync(_jobId, null, false, gsResult.ErrorMessage);
                    _resultSent = true;
                    closeAfterPrint = true;
                }
            }
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"打印失败:\n{ex.Message}",
                    "VirtualPrinter", MessageBoxButton.OK, MessageBoxImage.Error);
                await ServiceClient.SendResultAsync(_jobId, null, false, ex.Message);
                _resultSent = true;
                closeAfterPrint = true;
            }
            finally
